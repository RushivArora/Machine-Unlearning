...
class LoadData:
    ...
    def load_location(original_label):
        if original_label == "NY":
            df = pickle.load(open(PROCESSED_DATASET_PATH + "Insta_ny", 'rb'))
        elif original_label == "LA":
            df = pickle.load(open(PROCESSED_DATASET_PATH + "Insta_la", 'rb'))
        else:
    ...
    def load_adult(original_label):
        df = pickle.load(open(PROCESSED_DATASET_PATH + "adult", 'rb'))
    ...
    def load_accident(original_label):
        df = pickle.load(open(PROCESSED_DATASET_PATH + "accident", 'rb'))
        # 3-class balanced
    ...
    def load_mnist(batch_size=32):
        ...
        train_set = MNIST(root=ORIGINAL_DATASET_PATH + 'mnist', train=True, transform=transform,
                          download=True)
        test_set = MNIST(root=ORIGINAL_DATASET_PATH + 'mnist', train=False, transform=transform)
    ...
    def load_cifar10(batch_size=32, num_workers=1):
        ...
        train_set = CIFAR10(root=ORIGINAL_DATASET_PATH + 'cifar10', train=True, download=True, transform=transform_train)
        train_loader = DataLoader(train_set, batch_size=batch_size, shuffle=True, num_workers=num_workers)
        test_set = CIFAR10(root=ORIGINAL_DATASET_PATH + 'cifar10', train=False, download=True, transform=transform_test)
    ...
    def load_stl10(batch_size=32, num_workers=1):
        train_set = STL10(root=ORIGINAL_DATASET_PATH + 'stl10', split='train', download=True,
                          transform=transforms.Compose([
                              transforms.Pad(4),
                              transforms.Resize(32),
                              transforms.ToTensor(),
        ...
        test_set = STL10(root=ORIGINAL_DATASET_PATH + 'stl10', split='test', download=True,
                         transform=transforms.Compose([
                             transforms.ToTensor(),
                             transforms.Resize(32),
                             transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
...
class DataStore:
    ...
    def create_basic_folders(self):
        folder_list = [SPLIT_INDICES_PATH, SHADOW_MODEL_PATH, TARGET_MODEL_PATH,
                       ATTACK_DATA_PATH, ATTACK_MODEL_PATH]
    ...
    def determine_data_path(self):
        ...
        self.target_model_name = TARGET_MODEL_PATH + self.save_name
        self.shadow_model_name = SHADOW_MODEL_PATH + self.save_name

        self.attack_train_data = SHADOW_MODEL_PATH + "posterior" + self.save_name
        self.attack_test_data = TARGET_MODEL_PATH + "posterior" + self.save_name
    ...
    def save_record_split(self, record_split):
        pickle.dump(record_split, open(SPLIT_INDICES_PATH + self.save_name, 'wb'))

    def load_record_split(self):
        record_split = pickle.load(open(SPLIT_INDICES_PATH + self.save_name, 'rb'))
        return record_split
